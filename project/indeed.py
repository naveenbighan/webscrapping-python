...
jobs_data = []
index = 0
web_pages = int(input("Enter number of pages: "))
    ...
    if jobs:
        ...
        for post_name, company_name, location, time, summary in zip(post_names, company_names, locations, timings, summaries):
            ...
            try:
                ...
                print_time = time.strip('+23145') if any(term in time for term in terms) else "N/A"
                
                salary = time.strip('abcdefghijklmnopqrstuvwxyz ') if any(term in time for term in ["₹", "a year", "a month"]) else "N/A"
                ...
                # Append data to the list for CSV and MongoDB insertion
                jobs_data.append(all_job_data)
                df = pd.DataFrame(jobs_data)
                df.to_csv("indeed1_jobs.csv", index=False)
                
                # Insert into MongoDB
                # job_data_collection.insert_one(all_job_data)  # Uncomment to insert into MongoDB
